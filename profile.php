...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id']) && isset($_POST['name']) && isset($_POST['email']) && isset($_POST['password'])) {
        ...
        $stmt = $conn->prepare("UPDATE loginpage  SET name = ?, email = ?, password = ? WHERE id = ?");
        ...
        echo json_encode(["message" => "Invalid input or missing required fields (id, name, email, and password)."]);
    }
} else {
